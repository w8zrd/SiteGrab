...
// Static assets (index.html, script.js, styles.css) live at the repo root,
// not in a separate "public" folder — serve from __dirname.
...
// Identify honestly. Swap the contact URL for something real before deploying.
const USER_AGENT = process.env.CRAWLER_UA ||
  'sitegrab-crawler/1.0 (+https://example.com/sitegrab-bot; contact: you@example.com)';
...
// ---- Consent / audit log ------------------------------------------------
// Every job requires the submitter to attest they have the right to archive
// the target site. We log that attestation alongside the request so there's
// a record of who took responsibility for what.
const CONSENT_LOG_PATH = process.env.CONSENT_LOG_PATH || path.join(TMP_ROOT, 'consent-log.jsonl');

async function logConsent({ jobId, url, ip }) {
  const entry = {
    jobId,
    url,
    ip,
  ...
  try {
    await fsp.mkdir(path.dirname(CONSENT_LOG_PATH), { recursive: true });
    await fsp.appendFile(CONSENT_LOG_PATH, JSON.stringify(entry) + '\n');
  } catch (e) {
...
async function runWget(targetUrl, outDir) {
  return new Promise((resolve, reject) => {
    const args = [
      ...
      '--random-wait',
      '--wait=0.5',
      // Respect the target site's robots.txt / crawl-delay by default.
      // This tool identifies itself honestly (see USER_AGENT below) rather
      // than impersonating a browser, so this is a real crawler and should
      // behave like one.
      '-e', 'robots=on',
    ...
    child.on('close', (code) => {
      ...
      // quota hit, robots.txt disallow, etc). What matters is whether we got
      // any files at all.
      resolve({ code, stderr });
    });
  });
...
app.post('/api/jobs', async (req, res) => {
  const { url: rawUrl, consent } = req.body || {};
  if (!rawUrl) return res.status(400).json({ error: 'Missing url.' });
  if (consent !== true) {
    return res.status(400).json({ error: 'You must confirm you have the right to archive this site.' });
  }
  ...
  await logConsent({ jobId, url: parsed.toString(), ip: req.ip });
  res.json({ jobId });
    ...
    try {
      ...
      if (!gotFiles) {
        ...
        jobs.set(jobId, {
          ...jobs.get(jobId),
          status: 'error',
          error: 'wget did not download any files. The site may disallow crawling via robots.txt, be unreachable, or something failed on the server.' + (hint ? ` Details: ${hint}` : ''),
        });
